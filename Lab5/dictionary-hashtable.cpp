...
/* ha avuto successo.							    */
/* Ritorna FAIL se la chiave esisteva già o se  return ;  	 l'inserimento    */
...
Error dict::insertElem(const Key k, const Value v,  Dictionary& s)
{
	...
	aux->next = s[h(k)];
	/*
	if(s[h(k)] == emptyBucket) aux->next = emptyBucket;
	else  aux->next = s[h(k)];
	*/
	s[h(k)] = aux;
	return OK;
...
{	
	...
	while(aux!= emptyBucket){
		if(aux->elem.key == k) return aux->elem.value;
		aux = aux->next;
	}
	return emptyValue;
}
...
Error dict::deleteElem(const Key k, Dictionary& s)
{
	if (search(k,s) == emptyValue) return FAIL;
	Bucket aux = s[h(k)];
	Bucket aux1 = aux->next;
	if(aux->elem.key == k){ //Caso key in primo elemento
		delete aux;
		s[h(k)] = aux1;
		return OK;
	}
	while(aux1->elem.key != k  ){ //Altri casi
		aux1 = aux1->next;
		aux = aux->next;	
	}
	aux->next = aux1 ->next;
